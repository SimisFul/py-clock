...
class ClockSettings(object):
    ...
    FRAMERATE = 100  # None = unlimited fps
    FONT = "moonget.ttf"
    FULLSCREEN = False
    WINDOWED_WIDTH = 480
...
while en_fonction:
    ...
    if not toggle_menu:
        ...
        if animation_active_frame < animation_total_frames:
            ...
            if draw_from:
                pygame.draw.arc(surface, seconde_a_couleur(minute_changeante, inverser=True), rect_arc_minutes,
                                math.radians(90 - ((360 * (minute_changeante / (120.0 / draw_from)) / 60))),
        ...
        int((largeur / 2) + math.cos(math.radians(degree_secondes - 92)) * 135 * size_mult),
        int((hauteur / 2) + math.sin(math.radians(degree_secondes - 92)) * 135 * size_mult)))
        ecran.blit(texte, texte_rect)
